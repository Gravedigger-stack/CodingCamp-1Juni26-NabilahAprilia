...
This is a plain-HTML/CSS/Vanilla-JS project with no build step. The recommended testing approach uses:

- **[Jest](https://jestjs.io/)** with **[jsdom](https://github.com/jsdom/jsdom)** for unit and property tests (zero config, works without a build step via `jest --testEnvironment jsdom`)
- **[fast-check](https://fast-check.io/)** for property-based tests (imported as a dev-only script tag in the test harness or via Jest/CommonJS)
- Alternatively, a simple in-browser test runner (e.g., a `tests/runner.html`) using only `<script>` tags can be used if the project must stay build-tool-free

Each property-based test is configured to run a **minimum of 100 iterations** (`numRuns: 100` in fast-check).
...
tests/
  unit/
    ...
    timer.test.js        — Properties 6, 7, 8, 9
    todo.test.js         — Properties 10–16
    quicklinks.test.js   — Properties 20–23
    theme.test.js        — Properties 24, 25
    storage.test.js      — Properties 26, 27
    name.test.js         — Properties 4, 5
  integration/
    persistence.test.js  — End-to-end localStorage round trips
    init.test.js         — Widget boot order, default states
```
...
```js
// Feature: todo-life-dashboard, Property 3: Greeting matches the correct hour partition
test('greeting matches correct hour partition', () => {
  fc.assert(
    fc.property(fc.integer({ min: 0, max: 23 }), (hour) => {
      const greeting = _getGreeting(hour);
...
Unit tests cover concrete scenarios and edge cases that properties do not fully enumerate:

- Default timer display (`25:00`) on first load with no storage
- Start button disabled while timer is running
- Completion notification displayed at `00:00`
- OS `prefers-color-scheme` detection (dark/light, with mocked `matchMedia`)
- Quick Links max-50 boundary (exactly 50 links: add allowed; 51st: rejected)
- `localStorage` unavailable — write fails — notification shown
- Invalid `Date` object → `--:--:--` and `"Unknown date"` fallbacks

### Integration Test Focus Areas

- Full page boot sequence: all widgets render stored data before user interaction
- Cross-widget state independence: editing tasks does not affect timer or links
- Storage quota exceeded: mock `localStorage.setItem` to throw `QuotaExceededError`

### Accessibility and Cross-Browser

- Minimum font size (16 px) and touch target size (44 × 44 px) verified via manual review and CSS audit
- Functional testing in Chrome, Firefox, Edge, and Safari (latest stable) as per Requirement 10.4
- Screen reader smoke test: all interactive elements have accessible labels
